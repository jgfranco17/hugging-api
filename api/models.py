...
from .errors import ChatError, ModelOverloadedError
from .constants import MessageStatus, MessageType
...
class Message(Generator):
    ...
    def __next__(self) -> dict:
        ...
        try:
            ...
            if t == MessageType.STREAM:
                self.web_search_done = True
            elif t == MessageType.STATUS:
                pass
            elif t == MessageType.FINAL:
                self.text = a["text"]
                self.msg_status = MessageStatus.RESOLVED
            elif t == MessageType.WEB:
            ...
            if self._stream_yield_all or t == MessageType.STREAM:
                return a
            else:
